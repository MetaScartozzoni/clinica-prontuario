...
const NegotiationModal = ({ isOpen, onClose, negotiationData, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const updates = { 
        ...
        clinic_base_cost: parseFloat(formData.clinic_value) || 0,
        hospital_base_cost: parseFloat(formData.hospital_value) || 0,
        observation: formData.observation, 
        clinic_response: formData.clinic_response,
        updated_at: new Date().toISOString(),
    };

    if(formData.status === 'Em Negociação' && !negotiationData.budget.negotiation_started_at) {
        updates.negotiation_started_at = new Date().toISOString();
    } else if (formData.status === 'Aceito' && !negotiationData.budget.accepted_at) {
        updates.accepted_at = new Date().toISOString();
    } else if (formData.status === 'Recusado' && !negotiationData.budget.refused_at) {
    ...
    try {
      const negotiationRecord = {
        ...
        patient_id: negotiationData.budget.patient_id,
      ...
      if(formData.status === 'Aceito') {
        await supabase.rpc('create_financial_transaction_from_budget', { p_budget_id: negotiationData.budget.id });
      }
  ...
  if (!negotiationData?.budget && isOpen) {
    return (
